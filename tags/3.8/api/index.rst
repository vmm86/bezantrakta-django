...
В пакете ``api`` содержится текущее API проекта, используемое при оформлении заказов.

.. todo:: На данный API является "внутренним", т.е. используется только посетителями сайтов Безантракта в браузере и не предусмотрено для взаимодействия с любыми сторонними клиентами без использования браузера (на основе REST или подобного ему подхода). Однако при необходимости это можно реализовать, и к тому же переписать работу с API на основе **Django REST Framework** <http://www.django-rest-framework.org/>`_.
...
------------------------------------------
В силу специфики оплаты в СургутНефтеГазБанке результаты оплаты предварительно обрабатываются в ``sngb_proxy``, а затем происходит редирект на ``payment_handler`` (с помощью возврата стандартного HTML-ответа со строкой для редиректа).
