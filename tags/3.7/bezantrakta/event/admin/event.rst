...
* ``cancelled`` логическое значение - подпись "Отменено" поверх афиши, если **событие отменено**.

* ``rescheduled`` логическое значение  - подпись "Перенесено" поверх афиши, если **событие перенесено**.
...
* ``test`` логическое значение - афиши тестовых событий (имеющих этот параметр со значением ``true``) в любом случае не показываются на сайте (только в тестовой версии). На страницы тестовых событий можно заходить напрямую из админ-панели, нажимая на кнопку "*Смотреть на сайте*".
