...
class Command(BaseCommand):
    ...
    Поиск схем залов, групп и событий в подключенных к сайтам билетных сервисах.
    ...
    * либо только те схемы залов, ID которых указаны в настройках СПБ в параметре ``schemes``.
    ...
    """
    help = """
Поиск схем залов, групп и событий в подключенных к сайтам билетных сервисах.__
        ...
        if len(active_ticket_services) > 0:
            ...
            for ats in active_ticket_services:
                ...
                if schemes_inclusion_list is None:
                    ...
                    self.stdout.write('Из сервиса продажи билетов импортируются только схемы залов {vil}'.format(
                ...
                # Схемы залов из сервиса продажи билетов,
