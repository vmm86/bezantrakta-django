...
file_put_contents($logfile,  "init_response = " . print_r($_REQUEST, true) . "\n", FILE_APPEND);

$response = array_change_key_case($_REQUEST, CASE_LOWER);

$finalURL  = "REDIRECT=http://" . rtrim($_SERVER['SERVER_NAME'], '/');
$finalURL .= "/index.php?option=com_afisha&controller=gettickets&task=sngbInitProxy";
...
} // $finalURL .= "&result="       . mb_strtolower($response["result"], "UTF-8");
